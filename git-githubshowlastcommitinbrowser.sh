#!/usr/bin/env bash
# Usage: git-githubopenlastcommit
# geuss what it does

# stolen from
# https://raw.githubusercontent.com/cowboy/dotfiles/066a2bae6ae8a15b54c8db1c519725a9e4c074c4/source/50_vcs.sh
...
# alias gurlp='echo $(github_find_repo_url)/tree/$(gbs)/$(git rev-parse --show-prefix)'
...
#     /^[MA]\t/ {printf "%s\t$(github_find_repo_url)/blob/%s/%s\n", \$1, sha, \$2; next}
#     /.*/ {print \$0}
# AWK
#   )" | less -F
# }
...
function github_open_last_commit() {
  local n="${@:-1}"
  n=$((n-1))
  git web--browse  $(git log -n 1 --skip=$n --pretty=oneline | awk "{printf \"$(github_find_repo_url)/commit/%s\", substr(\$1,1,7)}")
}

github_open_last_commit
